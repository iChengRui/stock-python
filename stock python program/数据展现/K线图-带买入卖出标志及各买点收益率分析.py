...
def shouyiTable(cursor=None, stockname=None):
    ...
    QueryStr = """create temporary table Mairu select riqi,yuanyin from
                MaiRuBiao where stockname='""" + stockname + "' order by riqi"
    ...
    QueryStr = """create temporary table Maichu select distinct riqi from
                MaiChuBiao where stockname='""" + stockname + "' order by riqi"
    cursor.execute(QueryStr)
    QueryStr = "select Maichu.riqi,shoupanjia from Maichu left join " + stockname\
               + " on Maichu.riqi=" + stockname + ".riqi order by Maichu.riqi"
    cursor.execute(QueryStr)
...
class candlestickploter:
    def __init__(self, cur, stockindex=0):
        # 获取所有股票的代码
        self.cursor = cur
        self.stockcount = self.cursor.execute(
            "select stockname,MingCheng,LeiBie from XuanZe")
        # 序号从零开始，最后一个比stockcount小，才能保证+1时取到最后一个
        self.stockcount -= 1
    ...
    def GetInfo(self):
        ...
        while True:
            ...
            chengjiaoliang,riqi from """ + self.stocklist[self.stockindex][0]
            ...
            if len(self.Df) > 0:
                ...
                if self.stockindex < self.stockcount:
                    self.stocklist = np.delete(self.stocklist, self.stockindex,
                                               0)
                    self.stockcount -= 1
                else:
        ...
        self.QueryStr = "select riqi,yuanyin from MaiRuBiao where stockname='" + \
                        self.stocklist[self.stockindex][0] + "' order by riqi"
        ...
        # 对买入原因对应的收益进行统计
        self.shouyi = shouyiTable(cursor=self.cursor,
                                  stockname=self.stocklist[self.stockindex][0])
        # 将数据换算成以百分比%为单位的数据
        self.shouyi.values[:, 1:] = np.round(self.shouyi.values[:, 1:] * 100,
                                             2)
    ...
    def PlotIt(self):
        ...
        if len(self.Kongji) > 0:
            self.title = ",".join(self.Kongji) + u" 无历史数据，已跳过\n" + \
                         ...
                         self.stocklist[self.stockindex][2] + " " + str(
                self.stockindex)
        else:
            self.title = self.stocklist[self.stockindex][1] + " " + \
                         self.stocklist[self.stockindex][2] + " " + str(
                self.stockindex)
        ...
        self.ax3.table(cellText=self.shouyi.values, rowLabels=self.shouyi.index,
                       rowColours=None, colLabels=self.shouyi.columns.levels[1],
                       bbox=[0, 0, 1, 1])
