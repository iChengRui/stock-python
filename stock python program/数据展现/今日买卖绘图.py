...
class candlestickploter:
    ...
    def GetInfo(self):
        ...
        self.Kongji=[]
        while True:
            self.QueryStr="select kaipanjia,shoupanjia,zuigaojia,zuidijia,chengjiaoliang,riqi from " + self.stocklist[self.index]
            ...
            if len(self.Df)>0:
                ...
                if self.stockindex<self.stockcount:
                    ...
                    raise
        #获取股票的类别
        self.cursor.execute("select MingCheng,LeiBie from XuanZe where stockname='" + self.stocklist[self.index]+"'")
        self.Leibie=self.cursor.fetchall()
        #获取买入日期及原因
        self.QueryStr="select riqi,yuanyin from MaiRuBiao where stockname='" + self.stocklist[self.index] + "' order by riqi"
        ...
        #获取卖出日期及原因
        self.QueryStr="select riqi,yuanyin from MaiChuBiao where stockname='" + self.stocklist[self.index] + "' order by riqi"
        self.cursor.execute(self.QueryStr)
    ...
    def PlotIt(self):
        ...
        if len(self.Kongji)>0:
            self.title=",".join(self.Kongji)+ u" 无历史数据，已跳过\n" +self.stocklist[self.index]+self.Leibie[0][0]+" "+self.Leibie[0][1]+" "+str(self.index)
        else:
            self.title=self.Leibie[0][0]+" " +self.Leibie[0][1]+" "+str(self.index)
