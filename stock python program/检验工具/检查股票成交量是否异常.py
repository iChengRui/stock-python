...
while (stockindex < stockcount):
    QueryStr = "select max(chengjiaoliang),min(chengjiaoliang) from " + \
               stocklist[stockindex][0]
    ...
    if Df[0, 1] is not None:
        ...
        if Df[0, 0] / Df[0, 1] > 50:
            ...
            QueryStr = "select riqi from " + stocklist[stockindex][
                0] + " where chengjiaoliang=" + str(Df[0, 1])
            InfoRow = cursor.execute(QueryStr)
            T = cursor.fetchall()
